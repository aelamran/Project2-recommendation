...
ratings_test = splitting(test_data)
ratings_test['rating'] = test_data['Prediction']

y_test = np.zeros((ratings_test.shape[0], 5))
y_test[np.arange(ratings_test.shape[0]), ratings_test.rating - 1] = 1
...
movieid_test = ratings_test.movieid.values
userid_test = ratings_test.userid.values
...
# training at all.
print(mean_absolute_error(np.argmax(y, 1)+1, np.argmax(model.predict([movieid, userid]), 1)+1))
try:
    history = model.fit([movieid, userid], y,
                         nb_epoch=10,
                         validation_data=([movieid_test, userid_test], y_test))
...
print(mean_absolute_error(np.argmax(y_test, 1)+1, np.argmax(model.predict([movieid_test, userid_test]), 1)+1))
prediction = np.argmax(model.predict([movieid_test, userid_test]), 1)+1